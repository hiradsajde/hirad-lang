...
func parse_return_expr(p *parser) ast.Expr {
	p.advance()
	out := p.advance()
...
func parse_primary_expr(p *parser) ast.Expr {
	...
	case lexer.NUMBER:
		...
		var explicitType lexer.TokenKind
		p.dadvance()
		p.dadvance()
		if p.previousToken().Kind == lexer.INT {
			explicitType = lexer.INT
		}
		p.advance()
		p.advance()
		if explicitType == lexer.INT {
			conv, _ := strconv.ParseFloat(p.advance().Value, 64)
			number = int(conv)
		} else {
...
func parse_import_expr(p *parser) ast.Expr {
	if p.advance().Kind == lexer.INCLUDE {
		s := p.advance()
		if s.Kind == lexer.LESS {
			importName := p.expect(lexer.IDENTIFIER).Value
			p.advance()
			return ast.IncludeExpr{
				Name: importName,
			}
		} else if s.Kind == lexer.STRING {
			importName := s.Value
			return ast.IncludeExpr{
				Name: importName,
			}
	...
	panic("There is some issues with include")
}
